...
<!--
    ...
    Written by Renzo Sanchez-Silva <renzo@edac.unm.edu>
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/">
		<xsl:variable name="baseurl_prev" select="WMT_MS_Capabilities/Capability/Request/GetMap/DCPType/HTTP/Get/OnlineResource/@*[name()='xlink:href']"/>
		<xsl:variable name="format_prev" select="WMT_MS_Capabilities/Capability/Request/GetMap/Format"/>
        ...
        <xsl:apply-templates select="WMT_MS_Capabilities">
			...
			<xsl:with-param name="baseurl">
				<xsl:choose>
					<xsl:when test="contains($baseurl_prev,'?')"><xsl:value-of select="$baseurl_prev"/></xsl:when>
					<xsl:otherwise><xsl:value-of select="$baseurl_prev"/>?</xsl:otherwise>
				</xsl:choose>
			</xsl:with-param>
			<xsl:with-param name="format">
				<xsl:choose>
					<xsl:when test="$format_prev[translate(text(),$upper,$lower)='image/png']">image/png</xsl:when>
					<xsl:when test="$format_prev[translate(text(),$upper,$lower)='image/gif']">image/gif</xsl:when>
					<xsl:when test="$format_prev[translate(text(),$upper,$lower)='image/jpg']">image/jpg</xsl:when>
					<xsl:otherwise/>
				</xsl:choose>
			</xsl:with-param>
		</xsl:apply-templates>
