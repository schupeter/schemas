        ...
        <tr>
            ...
            <td class="summary">
                <xsl:for-each select="wadl:method">
                    ...
                    <a href="#{$id2}"><xsl:value-of select="$identifier"/></a>
    ...
    <xsl:if test="ancestor-or-self::wadl:*/wadl:param[@style=$style]">
				<xsl:choose>
						<xsl:when test="$style = 'template'">
							<div class="h7">paramètres de modèle</div>
						</xsl:when>
						<xsl:when test="$style = 'query'">
							<div class="h7">paramètres de la requête</div>
						</xsl:when>
				</xsl:choose>
        <table>
    ...
    <tr>
        <td><strong><xsl:value-of select="@name"/></strong></td>
            <td>
                ...
                <xsl:if test="@required = 'true'"><br/>(requis)</xsl:if>
...
<xsl:template name="getRepresentations">
    <xsl:if test="wadl:representation">
        <div class="h7">représentations</div>
        <table>
