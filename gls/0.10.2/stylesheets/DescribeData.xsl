...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gls="http://www.opengis.net/gls/0.10.2" xpath-default-namespace="http://www.opengis.net/gls/0.10.2" xmlns:ows="http://www.opengis.net/ows/1.1" xmlns:xlink="http://www.w3.org/1999/xlink">
			...
			<body>
				...
				<xsl:for-each select="gls:DataDescriptions/gls:Framework">
					...
					<table border="1" cellpadding="3">
						...
						<tr><td align="right"><i>Geolinkage Field:</i></td><td><xsl:value-of select="gls:GeolinkageField" /></td></tr>
						<tr><td align="right" valign="top"><i>Datasets:</i></td><td>
							<xsl:for-each select="gls:Dataset">
								<table border="1" cellpadding="3" align="center">
									...
									<tr><td align="right"><i>Authoring Organization:</i></td><td><xsl:value-of select="gls:Organization" /></td></tr>
									<tr><td align="right"><i>Release Date:</i></td><td><xsl:value-of select="gls:ReleaseDate" /></td></tr>
									<tr><td align="right" width="180"><i>Geolinkage Field:</i></td><td><xsl:value-of select="gls:GeolinkageField" /></td></tr>
									<tr><td align="right"><i>Records per Identifier:</i></td><td><xsl:value-of select="gls:GeolinkageRelationship" /></td></tr>
									<tr><td align="right"><i>Complete linkage:</i></td><td><xsl:value-of select="gls:GeolinkageComplete" /></td></tr>
   ...
   <table border="1" cellpadding="3">
	<xsl:for-each select="gls:Attribute">
		<xsl:choose>
			<xsl:when test="gls:Values/gls:Nominal">
				...
				<table width="100%" border="1" cellpadding="3">
					<tr  bgcolor="#cccccc"><td>Values</td><td></td></tr>
		...
		<tr><td colspan="3" align="center"><xsl:element name="a"><xsl:attribute name="href"><xsl:value-of select="$Service"/>Service=GDAS&amp;Version=<xsl:value-of select="/gls:DataDescriptions/@version" />&amp;Request=GetData&amp;FrameworkDomain=<xsl:value-of select="/gls:DataDescriptions/gls:Framework/gls:FrameworkDomain" />&amp;FrameworkName=<xsl:value-of select="/gls:DataDescriptions/gls:Framework/gls:FrameworkName" />&amp;FrameworkVersion=<xsl:value-of select="/gls:DataDescriptions/gls:Framework/gls:FrameworkVersion" />&amp;DatasetDomain=<xsl:value-of select="../gls:DatasetDomain" />&amp;DatasetName=<xsl:value-of select="../gls:DatasetName" />&amp;DatasetVersion=<xsl:value-of select="../gls:DatasetVersion" />&amp;Attributes=<xsl:value-of select="gls:AttributeName" />&amp;XSL=<xsl:value-of select="$XSLroot"/>/gdas2html2.xsl</xsl:attribute>View data</xsl:element></td></tr>
    </xsl:for-each>
   </table>
									</td></tr>
								</table>
